...
import { Laptop, Smartphone, CheckCircle, ArrowRight, Upload, X, ArrowLeft } from 'lucide-react';
...
const ClientDashboard = () => {
  ...
  const requests = (repairs || []).filter(r => ['Pending', 'In Process'].includes(r.status));
  ...
  const handleBooking = (e) => {
    ...
    addRepair(newReq);
    setStep(1);
    setBookingData({ device: '', brand: '', model: '', issue: '', address: '', image: null });
  };
    ...
    <div className="space-y-12 pb-20">
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-navy-100 overflow-hidden border border-gray-100 relative">
        <div className="bg-navy-900 px-10 py-12 text-center relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto">
             <h2 className="text-4xl md:text-5xl font-black tracking-tight text-white mb-4">Book Your Repair</h2>
             <p className="text-lg text-gray-400 font-medium">Fast, reliable service at your doorstep. Let's get your device running like new.</p>
          </div>
        </div>
        ...
        <div className="relative -mt-8 px-8 mb-12">
            <div className="bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-6 max-w-3xl mx-auto border border-gray-100 flex items-center justify-between relative z-20">
                ...
                <div key={s} className="flex-1 flex flex-col items-center relative">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-lg transition-all duration-500 relative z-10 ${
                    step >= s ? 'bg-lime-500 text-navy-900 shadow-lg shadow-lime-500/30 scale-110' : 'bg-gray-50 text-gray-300'
                    }`}>
                    {s}
                    </div>
                    <span className={`text-[10px] font-black uppercase tracking-widest mt-3 transition-colors duration-300 ${
                    ...
                    {s < 3 && (
                        <div className="absolute top-7 left-1/2 w-full h-1 bg-gray-100 -z-0">
                            <div className={`h-full bg-lime-500 transition-all duration-500 ${step > s ? 'w-full' : 'w-0'}`}></div>
                        </div>
                    )}
                </div>
                ))}
        ...
        <div className="px-8 pb-12 max-w-3xl mx-auto">
          <form onSubmit={handleBooking} className="animate-fade-in">
            {step === 1 && (
              <div className="space-y-8">
                <div className="text-center">
                    <span className="text-lime-500 font-extrabold text-xs uppercase tracking-widest">Step 1</span>
                    <h3 className="text-2xl font-black text-navy-900">What needs fixing?</h3>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  {['Mobile', 'Laptop'].map((type) => (
                    <button
                      key={type}
                      type="button"
                      onClick={() => { setBookingData({ ...bookingData, device: type }); setStep(2); }}
                    ...
                    >
                      <div className={`absolute top-0 right-0 p-8 opacity-10 transition-transform duration-500 group-hover:scale-110 ${bookingData.device === type ? 'text-lime-500' : 'text-navy-900'}`}>
                         {type === 'Mobile' ? <Smartphone className="w-40 h-40" /> : <Laptop className="w-40 h-40" />}
                      </div>
                      
                      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-all ${
                         bookingData.device === type ? 'bg-lime-500 text-navy-900' : 'bg-white text-navy-900 shadow-md group-hover:scale-110'
                      }`}>
                         {type === 'Mobile' ? <Smartphone className="w-8 h-8" /> : <Laptop className="w-8 h-8" />}
                      </div>
                      
                      <span className={`block font-black text-2xl mb-1 ${
                        bookingData.device === type ? 'text-white' : 'text-navy-900'
                      }`}>{type}</span>
                      <p className={`text-sm font-medium ${
                          bookingData.device === type ? 'text-gray-400' : 'text-gray-500'
                      }`}>
                          {type === 'Mobile' ? 'Screen, Battery, etc.' : 'Keyboard, Screen, etc.'}
                      </p>
                    </button>
                  ))}
                </div>
            ...
            {step === 2 && (
              <div className="space-y-8">
                 <div className="text-center">
                    <span className="text-lime-500 font-extrabold text-xs uppercase tracking-widest">Step 2</span>
                    <h3 className="text-2xl font-black text-navy-900">Device Details</h3>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                        ...
                        {BRANDS[bookingData.device].map(b => <option key={b} value={b}>{b}</option>)}
                        </select>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">Model</label>
                    <input
                      type="text"
                ...
                <div className="flex gap-4 pt-4">
                  <button type="button" onClick={() => setStep(1)} className="flex-1 py-4 bg-gray-100 text-gray-500 font-bold rounded-2xl hover:bg-gray-200 transition-all uppercase tracking-widest text-sm">Back</button>
                  <button type="button" onClick={() => setStep(3)} className="flex-1 py-4 bg-black text-white font-black rounded-2xl shadow-xl hover:bg-gray-900 hover:-translate-y-1 transition-all uppercase tracking-widest text-sm flex items-center justify-center gap-2" disabled={!bookingData.brand || !bookingData.model}>
            ...
            {step === 3 && (
              <div className="space-y-8">
                <div className="text-center">
                    <span className="text-lime-500 font-extrabold text-xs uppercase tracking-widest">Step 3</span>
                    <h3 className="text-2xl font-black text-navy-900">Problem & Contact</h3>
                </div>

                <div className="space-y-6">
                    <div className="space-y-2">
                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">Describe Issue</label>
                        <textarea
                    ...
                    <div className="space-y-2">
                         <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">Pickup Address</label>
                        <textarea
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">Photo (Optional)</label>
                        <div className="border-2 border-dashed border-gray-200 rounded-3xl p-8 text-center bg-gray-50 group cursor-pointer hover:border-lime-500 hover:bg-lime-50/10 transition-all duration-300">
                            ...
                            <p className="text-gray-400 font-medium text-sm">Click to upload photo of damage</p>
                        </div>
                    </div>
                </div>

                <div className="flex gap-4 pt-4">
                  <button type="button" onClick={() => setStep(2)} className="flex-1 py-4 bg-gray-100 text-gray-500 font-bold rounded-2xl hover:bg-gray-200 transition-all uppercase tracking-widest text-sm">Back</button>
                  <button type="submit" className="flex-1 py-4 bg-black text-white font-black rounded-2xl shadow-xl hover:bg-gray-900 hover:-translate-y-1 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-sm">
      ...
      <div className="space-y-8">
        <div className="flex items-end justify-between px-4">
           <div>
              <h2 className="text-3xl font-black text-navy-900">Active Repairs</h2>
              <p className="text-gray-500 font-medium">Track the live status of your devices</p>
           </div>
           <Link to="/client/history" className="text-navy-900 font-bold flex items-center text-sm hover:text-lime-600 transition-colors uppercase tracking-wide bg-white px-5 py-2.5 rounded-xl shadow-sm border border-gray-100 mb-1">
        ...
        <div className="grid grid-cols-1 gap-6">
          {requests.map((req) => (
            <div key={req.id} className="bg-white p-8 rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 hover:shadow-2xl hover:shadow-gray-200 transition-all group relative overflow-hidden">
              <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-gray-50 to-gray-100 rounded-bl-[100px] -mr-8 -mt-8 transition-transform group-hover:scale-150`}></div>
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center gap-8">
                <div className="flex items-start gap-6 md:w-[280px] lg:w-[320px] shrink-0">
                  <div className={`w-20 h-20 rounded-2xl flex items-center justify-center shadow-lg shrink-0 ${
                    ...
                    {req.device === 'Mobile' ? <Smartphone className="w-10 h-10" /> : <Laptop className="w-10 h-10" />}
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <h3 className="font-black text-xl text-navy-900 truncate">{req.brand} {req.model}</h3>
                    </div>
                    <div className="flex items-center gap-2 mb-3">
                        <span className="bg-gray-100 text-gray-500 px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider shrink-0">#{req.id}</span>
                        <p className="text-gray-500 font-medium text-sm truncate">{req.issue}</p>
                    </div>
                    <div className="flex items-center gap-3">
                         <span className={`px-3 py-1 rounded-full text-[10px] font-bold border flex items-center gap-2 ${statusColors[req.status]}`}>
                ...
                <div className="flex-1 bg-gray-50 p-6 rounded-2xl border border-gray-100 w-full">
                  ...
                  {req.vendor && (
                     <div className="mt-4 flex items-center gap-2 pt-4 border-t border-gray-200">
                        <div className="w-6 h-6 rounded-full bg-navy-900 text-white flex items-center justify-center text-[10px] font-bold">T</div>
                        <div className="text-xs text-navy-900 font-bold">Tech: {req.vendor}</div>
                     </div>
                   )}
                </div>

                <div className="flex flex-row md:flex-col gap-3">
                   <button className="flex-1 md:flex-none p-4 bg-white border border-gray-100 rounded-xl text-gray-400 hover:text-red-500 hover:border-red-100 hover:bg-red-50 transition-all shadow-sm">
             ...
             <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-slide-up relative">
                 ...
                 <div className="p-10 pb-0">
                     ...
                     <p className="text-gray-500 font-medium flex items-center gap-2">
                         <ArrowRight className="w-4 h-4 text-lime-500" /> Submitted on {selectedRepair.date}
                     </p>
                 </div>
